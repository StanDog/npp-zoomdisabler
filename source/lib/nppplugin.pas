...
uses
  Winapi.Windows, Winapi.Messages, SciSupport, System.SysUtils,
...
begin
  ...
  if (HWND(sn^.nmhdr.hwndFrom) = self.NppData.NppHandle) and (sn^.nmhdr.code = NPPN_READY) then
  begin
    self.DoNppnReady;
  end;
  // @todo
...
begin
  ...
  else if (Msg.Msg = WM_ACTIVATE) then
  begin
    // TODO: Find out how to react to activate messages
    // to bring settings and about dialogs to front because
    // they move to background and since they have fsStayOnTop
    // the NPP main window will not respond anymore!
  end;
...
var
  curScintilla: Integer;
begin
  SendMessage(self.NppData.NppHandle, NPPM_GETCURRENTSCINTILLA, 0, LPARAM(@curScintilla));  //LPARAM(@curScintilla)
